...
module ActiveSupport
  ...
  module Rescuable
    ...
    module ClassMethods
      ...
      def rescue_from(*klasses, with: nil, cause: true, &block)
        unless with
          if block_given?
            with = block
          else
        ...
        klasses.each do |klass|
          ...
          self.rescue_handlers += [[key, with, !!cause]]
        end
      end
      ...
      private
        def find_rescue_handler(exception, from_cause = false)
          if exception
            # Handlers are in order of declaration but the most recently declared
            # is the highest priority match, so we search for matching handlers
            # in reverse.
            _, handler, _ = rescue_handlers.reverse_each.detect do |class_or_name, _, match_cause|
              next if from_cause && !match_cause
